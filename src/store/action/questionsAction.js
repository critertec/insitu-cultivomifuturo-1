...
export const nextSectionLuisa = () => async (dispatch, getStore) => {
  ...
  if (listSectionAlternal.length < 6) {
    const numberSectionActive = getRandomInt(1, 15, listSectionAlternal);
...
export const instructionsClose = () => async (dispatch, getStore) => {
  ...
  const numberSectionActive =
    etapa === "tutorial" ? 1 : getRandomInt(1, 15, listSectionAlternal);
...
export const welcomeYeinerTutorial = () => async (dispatch, getState) => {
  ...
  const quesAct =
    subQuestion === "b"
      ? questionActive + 1
      : questionActive === 3 && subQuestion === "a"
      ? questionActive
      : 1;
  ...
  const subQues =
    subQuestion === "b"
      ...
      ? "b"
      : "a";
  const actInit = questionActive >= 2 ? "inicio" : "welcome";
  const actInitb = quesAct >= 4 && "welcome";
